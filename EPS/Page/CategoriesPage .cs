...
namespace EPS.PL
{
    public partial class CategoriesPage : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void LoadData()
        {
            ...
            dbContext.TB_Categories.LoadAsync().ContinueWith(loadTask =>
            {
                gridControl1.DataSource = dbContext.TB_Categories.Local.ToBindingList();
        ...
        private void btn_edit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (id != 0)
                {
                    add = db.TB_Categories.Where(x => x.ID == id).FirstOrDefault();
                    Add_Categories page = new Add_Categories();
                    page.btn_add.Text = "تعديل";
                    page.btn_addclose.Text = "تعديل + غلق";
                    page.edt_name.Text = add.CatName;
                    page.combo_groups.SelectedItem = add.CatGroup;
            ...
            try
            {
                ...
                if (id != 0)
                {
                    ...
                    if (dilogeresult == DialogResult.Yes)
                    {
                        ...
                        var Note = " تم حذف صنف من قبل " + username;
