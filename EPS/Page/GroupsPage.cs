...
namespace EPS.PL
{
    public partial class GroupsPage : DevExpress.XtraEditors.XtraUserControl
    {
        DBEPSEntities db;
        TB_Groups add;
        public int id;
        ...
        {
            EPS.DBEPSEntities db = new EPS.DBEPSEntities();
            db.TB_Groups.LoadAsync().ContinueWith(loadTask =>
            {
                // Bind data to control when loading complete
                gridControl1.DataSource = db.TB_Groups.Local.ToBindingList();
        ...
        private void btn_edit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (id != 0)
                {
                    add = db.TB_Groups.Where(x => x.ID == id).FirstOrDefault();
                    Add_Groups page = new Add_Groups();
                    page.btn_add.Text = "تعديل";
                    page.btn_addclose.Text = "تعديل + غلق";
                    page.edt_name.Text = add.GroupName;
                    page.id = id;
                    page.page = this;
                    page.Show();
                }
            ...
            try
            {
                ...
                if (id != 0)
                {
                    ...
                    if (dilogeresult == DialogResult.Yes)
                    {
                        ...
                        var Note = " تم حذف مجموعة من قبل " + username;
